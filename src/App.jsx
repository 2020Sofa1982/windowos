...
const TX={
  ru:{
    // Dashboard
    realData:"Реальные данные из системы",
    todayWidget:"⚡ Сегодня",
    overdueFollowUp:"🔴 Просроченные follow-up",
    todayFollowUp:"🟡 Follow-up сегодня",
    todayMeasures:"📐 Замеры сегодня",
    todayInstalls:"🔧 Монтажи сегодня",
    pendingPaymentsToday:"💰 Ожидаются платежи",
    totalLabel:"Итого",
    revenueByMonth:"📈 ВЫРУЧКА ПО МЕСЯЦАМ",
    contracted:"КП сумма",
    receivedChart:"Получено",
    profitChart:"Прибыль",
    noOrdersYet:"Заказы появятся здесь после добавления",
    conversionFunnel:"🎯 ВОРОНКА КОНВЕРСИИ",
    totalLeads:"Лидов всего",
    measures:"Замеров",
    ordersCount:"Заказов",
    completed:"Завершено",
    leadToMeas:"Лид→Замер",
    measToOrder:"Замер→Заказ",
    leadToOrder:"Лид→Заказ",
    avgDeal:tx("avgDeal"),
    ...
    ofTotal:"из",
    pendingPaymentsHdr:"💰 ОЖИДАЮТСЯ ПЛАТЕЖИ",
    statusHdr:"🔔 СТАТУС",
    ...
    searchMeasurements:"Поиск по замерам...",
    ...
    measurementNotes:"Заметки по монтажу",
    sendToCalc:"Отправить в калькулятор",
    // Orders page
    orderId:"ID заказа",
    searchOrders:"Поиск по клиенту или ID...",
    ...
    searchPayments:"Поиск по клиенту...",
    ...
    itemNameReq:"Наименование *",
    unit:"Ед.",
    qty:"Кол-во",
    minQty:"Минимум (сигнал)",
    ...
    other2:"Прочее",
    variableCosts:"Разовые расходы",
    costNotes:"Заметки по затратам...",
    revenueNeeded:"Нужно выручки",
    byMonth:"По месяцам",
    // Calc page
    quoteSaved:"КП сохранено в историю!",
    orderCreated:"Заказ создан! Лид обновлён.",
    specifyClient:tx("specifyClient"),
    detailedCalc:"Детальный расчёт",
    itemTotal:"Итог по позиции",
    baseDekel:"База Dekel",
    accessories:"Аксессуары",
    colorLabel:"Цвет профиля",
    screen:"Сетка",
    shutter:"Роллет",
    shutterShutterShort:"Роллет / тришс",
    installFactor:"× монтаж",
    selfCostPer1:"Себест. за 1 шт",
    installation:"Монтаж",
    surcharges:"Доплаты",
    shutterCoeff:"Коэфф. роллет (рынок vs Dekel)",
    recommended:"Рекомендовано",
    marginPct:"Маржа %",
    selfCost:"Окна (себест.)",
    surchTotal:"Доплаты",
    totalSelfCost:"Итого себест.",
    ...
    scenarios:"3 сценария",
    pessimist:"Пессимизм ×1.10",
    base:"База ×1.13",
    optimist:"Оптимизм ×1.17",
    bomTitle:"📦 BOM — Список материалов",
    profileLm:"Профиль м.п.",
    weightKg:"Вес кг",
    glassSqm:"Стекло м²",
    screens:"Сетки",
    shutters:"Роллеты",
...
const MOP_T=[
  ...
  {value:"Хаза 3-трек",        label:bi("Хаза 3-трек","הזזה 3 מסלולים","Sliding 3-track")},
  {value:"Поворотно-откидное", label:bi("Поворотно-откидное","כיפ-כיפ","Tilt-turn")},
  {value:"Откидное",            label:bi("Откидное","כיפ","Tilt-only")},
  {value:"Глухое",              label:bi("Глухое","פיקס","Fixed")},
  {value:"Карман (кис)",        label:bi("Карман (кис)","הזזה לכיס","Pocket sliding")},
  {value:"Бельгийское",         label:bi("Бельгийское","בלגי","Belgian")},
  {value:"Дверь хаза",          label:bi("Дверь хаза","דלת הזזה","Sliding door")},
  {value:"Дверь поворот",       label:bi("Дверь поворот","דלת ציר","Pivot door")},
];
const WT=[
  {value:"Железобетон",label:bi("Железобетон","בטון מזוין","Reinforced concrete")},
  {value:"Кирпич",     label:bi("Кирпич","לבנים","Brick")},
  {value:"Газобетон",  label:bi("Газобетон","בלוק מקציף","Aerated concrete")},
  {value:"Блоки",      label:bi("Блоки","בלוקים","Blocks")},
  {value:"Дерево",     label:bi("Дерево","עץ","Wood")},
  {value:"Другое",     label:bi("Другое","אחר","Other")},
];
...
const Sel=({label,options,...p})=>(<div style={{marginBottom:12}}>
  ...
  <select {...p} style={{width:"100%",background:D.bg,border:`1px solid ${D.border}`,borderRadius:8,
    ...
    {options.map(o=>typeof o==="string"?<option key={o} value={o} style={{background:D.card}}>{o}</option>
      :<option key={o.value} value={o.value} style={{background:D.card}}>{t(o.label)}</option>)}
  </select>
</div>);
  ...
  const rows=validItems.map((c,i)=>{
    ...
    let html=`<tr>
      ...
      <td><b>${c.name}</b>
        ...
        <br/><span class="sub">צבע: ${t(colorOpt.name)} ${colorSwatch} ${colorOpt.ral}</span>
      </td>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <Btn onClick={()=>{
          ...
          calced.filter(c=>c.valid).forEach(c=>{
            ...
            rows.push([c.name,t(PNAMES[c.profile])||c.profile,profM+" м.п.",glassM2+" м²",sealM+" м.п.",
...
function FinancePL({orders,payments,leads,measurements,kpi}){
  const [fixedCosts,setFixedCosts]=useState(()=>load("wb:fixedcosts",[
    {id:1,name:"Аренда",amount:3000},
    {id:2,name:"Электроэнергия",amount:500},
    {id:3,name:"Бухгалтер",amount:800},
    {id:4,name:"Телефон/Интернет",amount:300},
    {id:5,name:"Транспорт",amount:1200},
  ]));
